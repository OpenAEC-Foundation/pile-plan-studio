...
describe("pile plan workspace legend editing", () => {
  it("applies activation and appearance in one project update", () => {
    ...
    const applyBlock = source.match(/onApply=\{\(draft, enableTipLevelRegions\) => \{(?<body>[\s\S]*?)setLegendEditorOpen\(false\);/)
      ?.groups?.body ?? "";
    ...
    assert.match(applyBlock, /showTipLevelRegions:\s*enableTipLevelRegions \? true : state\.showTipLevelRegions/);
