...
describe("compact application baseline", () => {
  it("uses root-level compact zoom only in the browser runtime", () => {
    assert.match(css, /html\.browser-compact-baseline\s*\{[\s\S]*?zoom:\s*0\.8/);
    assert.match(main, /applyRuntimeBaseline\(isDesktopRuntime\(\)\)/);
