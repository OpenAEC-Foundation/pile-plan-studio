...
describe("pile plan file import", () => {
  ...
  it("creates and activates a new plan without replacing the current plan", () => {
    ...
    assert.deepEqual(next.pilePlans[1].activePileTipLevelMms, [-18]);
    assert.notEqual(next.pilePlans[1].activePileSizes, currentPlan.activePileSizes);
    assert.notEqual(next.pilePlans[1].activePileTipLevelMms, currentPlan.activePileTipLevelMms);
