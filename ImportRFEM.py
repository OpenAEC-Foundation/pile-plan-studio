...
def import_rfem():
    coordinatensheet = np.array(pd.read_excel(excel_file, sheet_name=0))[1:]
    belastingensheet = np.array(pd.read_excel(excel_file, sheet_name=1))[1:]
...
def write_csv(knopen, coordinaten, belastingen):
    with open('Belastinglocaties.csv', mode='w', 
           newline = '') as file:
