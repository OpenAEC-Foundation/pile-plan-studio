...
| `apps/pile-plan-studio/src/App.tsx`, `app/session/`, and `app/*Controller.ts` | Bootstrap the core and settings, compose the active session, and coordinate request and project lifecycles | TypeScript workflow | **Application-owned.** File open, sample open, import, refresh, save, and recovery use the shared project-document contract; gesture grouping, request lifetimes, dirty-state baseline, and persistence timing remain interface workflow. Controllers reject stale outcomes but do not reinterpret core results. |
