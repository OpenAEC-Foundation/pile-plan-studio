...
| `apps/pile-plan-studio/src/core/projectFile.ts` | IFCPP wire types, schemas 1–4 branching, defaults, normalization, hydration, and save-document construction | Rust for interpretation; TypeScript for mechanical hydration | **Migration target.** Remove schema branching, engineering defaults, validation, canonical construction, and float-derived identity. Retain record/Map conversion, immutable copies, import summaries, and presentation mapping. |
...
| `apps/pile-plan-studio/src/domain/openedProject.ts` | Coordinates core read, a second position validation call, and state creation | TypeScript workflow | **Migration target.** Retain atomic open orchestration; remove redundant position validation after canonical Rust read validates the complete project. |
...
| `apps/pile-plan-studio/src/domain/projectContent.ts` | Captures/restores undoable content, calculates analysis invalidation, constructs IFCPP for persistence | TypeScript for history; Rust for project construction | **Migration target.** Retain content snapshots, restoration, and invalidation. Replace IFCPP construction with extraction of a domain-document draft. |
...
| `apps/pile-plan-studio/src/domain/browserRecovery.ts` | Recovery envelope plus shallow independent IFCPP parsing/schema checks | TypeScript envelope; Rust project validation | **Migration target.** Retain envelope validation and metadata, but do not treat its JSON inspection as project validation or maintain an independent supported-schema list. |
...
| `apps/pile-plan-studio/src/App.tsx` | Connects project state, core calls, save baseline, dirty state, and recovery scheduling | TypeScript workflow | **Application-owned.** Replace old read/write calls but retain gesture grouping, request lifetimes, dirty-state baseline, and persistence timing. |
...
| `apps/pile-plan-studio/src/domain/pileCostCatalog.ts` | Row validation messages, immutable catalog edits, display partitioning, and merging personal/built-in defaults | Split | **Migration target** for persisted project validity and silent repair. Immediate editor feedback, immutable edits, display partitioning, and preference merging remain application-owned. |
